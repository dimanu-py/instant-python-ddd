# Spec Generation Standard

## Convention

Feature specifications are captured in `.md` spec files inside `docs/specs/`. A spec captures a feature's purpose, behavior, contract, edge cases, and decisions before any code is written. This project follows a lightweight Kiro-style flow: purpose/behavior/contract → EARS requirements → tasks → code. Code is not written until the spec is approved by a human.
...
- Forces contract clarity before code exists, making the human approval gate cheap and high-leverage.
- EARS requirements are directly convertible into executable tests, keeping the spec testable.
- Recorded decisions and discarded alternatives prevent re-litigating settled design choices.
...
```markdown
WHEN a user submits a table with duplicate names, the system SHALL reject the submission
and SHALL NOT create any rows.
```

### Bad: Soft verb and mixed SHALLs

```markdown
The system could support importing CSV, Excel, and JSON files, and should validate the data.
```

## Real world examples

- Existing specs live in `docs/specs/`

## Related agreements

- `docs/agents/convention_guidelines.md` — document structure standard
- `docs/agents/leader_workflow.md` — pipeline and approval gate
