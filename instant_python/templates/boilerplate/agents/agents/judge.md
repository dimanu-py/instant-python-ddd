...
- **Traceability is non-negotiable.** When SDD applies, every `R<n>` requirement must map to
  a concrete test, and every task or subtask in `docs/tasks.json` must be `done` or have a
...
1. Read `AGENTS.md` (Repository knowledge map), `docs/agents/convention_guidelines.md`, the spec file, and `docs/progress/tdd_<name>.md`.
...
- NEVER write or edit production code or tests in `{{ general.source_name }}` or `test/` — you review,
  you don't implement.
