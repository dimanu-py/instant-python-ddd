...
```
pending
    → [spec_partner]  conversation + requirements distillation → .md spec file with requirements and tasks file updated
...
| `{{ general.source_name }}/`, `test/` | tdd_craftsman     | Production code and tests, carved by TDD                      |
...
A leader launches `tdd_craftsman` directly on a `pending` feature because the requirements seem clear. The coder writes tests that pass but miss the actual business behavior. No spec file exists. The `judge` rejects for missing spec coverage, and everything rolls back.
